...
// Models
import { Leader } from '@/redux/admin/leaders/adminLeadersApi';
// Utils
import { getCategoryImageUrl, getLeaderImage } from '@/utils/imageHandler';

interface LeaderViewModalProps {
    leader: Leader | null;
    isOpen: boolean;
    onClose: () => void;
}

export const LeaderViewModal: React.FC<LeaderViewModalProps> = ({
    leader,
    isOpen,
    onClose
}) => {
    ...
    if (!isAnimating || !leader) return null;
                ...
                >
                    ...
                    <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                        <div className="flex items-center space-x-2 sm:space-x-3">
                            ...
                            <div>
                                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Leader Details</h2>
                                <p className="text-xs sm:text-sm text-gray-500 hidden sm:block">View leader information</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                        ...
                        <div className="space-y-4 sm:space-y-6 md:space-y-8">
                            ...
                            <div className="text-center">
                                <div className="inline-block">
                                    <div className="w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl sm:rounded-3xl flex items-center justify-center overflow-hidden border-2 sm:border-4 border-white shadow-xl sm:shadow-2xl hover:shadow-3xl transition-all duration-300 hover:scale-105">
                                        {leader.image ? (
                                            ...
                                            <div className="text-center">
                                                <User className="w-8 h-8 sm:w-12 sm:h-12 md:w-16 md:h-16 text-gray-400 mx-auto mb-1 sm:mb-2" />
                                ...
                                <div className="mt-3 sm:mt-4">
                                    <h3 className="text-xl sm:text-2xl font-bold text-gray-900 break-words">{leader.fullName}</h3>
                                    <p className="text-gray-500 mt-1 text-sm sm:text-base">{leader.category?.name || 'No Category'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                                ...
                                <div className="space-y-2 sm:space-y-3 group">
                                    ...
                                    <div className="p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl border border-blue-100 group-hover:shadow-md transition-all duration-200">
                                        <p className="text-gray-900 font-medium text-sm sm:text-base break-words">{leader.fullName}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 sm:space-y-3 group">
                                    ...
                                    <div className="p-3 sm:p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg sm:rounded-xl border border-green-100 group-hover:shadow-md transition-all duration-200">
                                        <div className="flex items-center space-x-2">
                                            {leader.category?.image && (
                                                <img
                                                    src={getCategoryImageUrl(leader.category.image)}
                                                    alt={leader.category.name}
                                                    className="w-6 h-6 rounded-full object-cover"
                                                />
                                            )}
                                            <p className="text-gray-900 font-medium text-sm sm:text-base">
                                                {leader.category?.name || 'No Category Assigned'}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 sm:space-y-3 group">
                                    ...
                                    <div className="p-3 sm:p-4 bg-gradient-to-r from-gray-50 to-slate-50 rounded-lg sm:rounded-xl border border-gray-100 group-hover:shadow-md transition-all duration-200">
                                        <span className={`inline-flex items-center px-2 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 ${leader.isDeleted
                                            ? 'bg-red-100 text-red-800 hover:bg-red-200'
                                            : 'bg-green-100 text-green-800 hover:bg-green-200'
                                            }`}>
                                            {leader.isDeleted ? 'Deleted' : 'Active'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 sm:space-y-3 group">
                                    ...
                                    <div className="p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-lg sm:rounded-xl border border-purple-100 group-hover:shadow-md transition-all duration-200">
                                        <p className="text-gray-900 text-sm sm:text-base">
                                            {new Date(leader.createdAt).toLocaleDateString('en-US', {
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                                            })}
                            ...
                            <div className="space-y-2 sm:space-y-3 group">
                                ...
                                <div className="p-4 sm:p-6 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg sm:rounded-xl border border-orange-100 min-h-[80px] sm:min-h-[120px] group-hover:shadow-md transition-all duration-200">
                                    {leader.description ? (
                                        <p className="text-gray-900 leading-relaxed text-sm sm:text-base break-words">{leader.description}</p>
                                    ) : (
                            ...
                            <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-lg sm:rounded-xl p-4 sm:p-6 border border-gray-100 hover:shadow-md transition-all duration-200">
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:gap-6 text-xs sm:text-sm">
                                    <div className="space-y-2">
                                        <span className="text-gray-500 font-medium">Leader ID:</span>
                                        <p className="font-mono text-gray-900 break-all bg-white p-2 sm:p-3 rounded-lg border text-xs sm:text-sm">{leader._id}</p>
                                    </div>
                                    {leader.category && (
                                        <div className="space-y-2">
                                            <span className="text-gray-500 font-medium">Category ID:</span>
                                            <p className="font-mono text-gray-900 break-all bg-white p-2 sm:p-3 rounded-lg border text-xs sm:text-sm">{leader.category._id}</p>
                                        </div>
                                    )}
                                    <div className="space-y-2">
                                        <span className="text-gray-500 font-medium">Last Updated:</span>
                                        <p className="text-gray-900 bg-white p-2 sm:p-3 rounded-lg border text-xs sm:text-sm">
                                            {new Date(leader.updatedAt).toLocaleDateString('en-US', {
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric',
                                                hour: '2-digit',
